"""Audit class-ratio aggregate summaries against raw predictions."""
...
def audit_class_ratio_result_summary(summary_path: str | Path) -> dict[str, Any]:
    """Audit a class-ratio aggregate summary against raw prediction CSV files."""

    with Path(summary_path).open("r", encoding="utf-8") as file:
        summary = json.load(file)

    if summary.get("status") != "completed":
        raise ResultAuditError(f"Class-ratio summary is not completed: {summary_path}")
    if "ratios" not in summary:
        raise ResultAuditError("Class-ratio summary missing ratios section")

    seed_audits = [_audit_seed_result(seed) for seed in summary["seed_results"]]
    ratio_model_audits = {}
    for ratio_id, model_summaries in summary["ratios"].items():
        ratio_model_audits[ratio_id] = {}
        for model_id, model_summary in model_summaries.items():
            matching_audits = [
                audit
                for audit in seed_audits
                if audit["ratio_id"] == ratio_id and audit["model_id"] == model_id
            ]
            ratio_model_audits[ratio_id][model_id] = _audit_model_summary(
                model_summary,
                matching_audits,
            )
    ...
    return {
        ...
        "ratio_model_audits": ratio_model_audits,
...
def _audit_seed_result(seed_result: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "match_to_non_match_ratio": seed_result["match_to_non_match_ratio"],
...
def _audit_model_summary(
    ...
    if not seed_audits:
        raise ResultAuditError("Missing seed audits for ratio/model summary")
